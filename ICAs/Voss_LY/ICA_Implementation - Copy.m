% Liyan records: this function is the original one from Voss. 
%   I update this function to realize our own.
%
% 05-06-2016
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    demixing formula.
    ...
    elseif strcmp(preprocessingChoice, 'pseudo-euclidean ip')
        ...
        % LY center: mean subtract data
        b = mean(X, 2);
        % X = X - repmat(b, 1, N);
        %% <-- TMP for testing LY C
        
        % centering (have it originally)
        X = bsxfun(@minus, X, b);
        
        % construct the inner product space
        C = pseudoEuclideanIP(X, functionChoice);
        
%         % try LY's idea of new C
%         C = InnerC_fun(X);
